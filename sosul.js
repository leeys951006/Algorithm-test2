...
console.log(`1. 위 sosulText 라는 문자열의 데이터 크기는 몇입니까? : ${new Blob([sosulText]).size} bytes`);
const countNa = (sosulText.match(/이연승/g) || []).length;
console.log(`2. 위 sosulText 에 존재하는 '나' 의 이름은 몇번 언급되었습니까? : ${countNa}번`);
const words = sosulText.replace(/[^\w\s]/g, '').split(/\s+/);
...
console.log(`3. 가장 많이 언급된 단어는 무엇입니까? : '${mostFrequentWord}'`);
const newName = '정호연';
const newSosulText = sosulText.replace(/이연승/g, newName);
console.log('4. 극중 나의 배역이 마음에 들지 않습니다. 다른이름으로 배역을 변경해주세요 :');
console.log(newSosulText);
const badPerson = '공욱재';
const badPersonCount = (sosulText.match(new RegExp(badPerson, 'g')) || []).length;
console.log(`5. 극중에 가장 나쁜사람은 누구입니까? : '${badPerson}'`);
